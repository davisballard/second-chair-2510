# Second Chair — Roles & Compensation

**Equal Co-Founders | 33/33/33 Profit Split**

---

## Founding Team

Three equal co-founders. Every dollar of profit splits evenly. No hierarchy — three domain owners who complement each other.

---

### Davis Ballard — Co-Founder & Creative & Strategy Systems

**What He Owns:**

*Core (Day 1):*
- Brand positioning, strategy, and messaging architecture
- Ad creative development and production (Meta, Google, TikTok, Snapchat, LinkedIn)
- Email, website copy, and content strategy
- Campaign concepting, scripts, visual direction, art direction
- Creative direction for Fair Case consumer brand
- Behavioral science integration into creative and messaging
- Technical sales support for large accounts (closing, not cold calling)

*Expansion (Month 4–6):*
- Email / nurture flow builds for attorney partners
- Brand identity projects for partner firms
- Ad campaign creative for partner firms (creative side)
- Content strategy for partner firms
- Premium lead nurture copy and sequences

**Background:** 18 years across strategy and creative for Disney, Google, Samsung, NFL, NBA, Uber, Taco Bell, The Macallan, Oculus, Optum, and 12 Fortune 500 brands. Partnered with 55-time Cannes Lion Winner Julian Cole on 4 Fortune 500 strategy projects. Built Second Chair's AI-powered marketing engine integrating marketing science, behavioral psychology, and creative execution.

**Location:** Los Angeles / San Diego, California

**Profile:** [01_Identity/employees/DavisBallard.md](../01_Identity/employees/DavisBallard.md)

---

### Alex DuBelko — Co-Founder & Product & Compliance Systems

**What He Owns:**

*Core (Day 1):*
- Website development, design, and optimization
- Consumer-facing funnel architecture (quiz flows, intake forms, landing pages)
- Internal tools and product development
- CRM architecture and lead scoring/routing system design
- Analytics infrastructure and reporting dashboards
- A/B testing and conversion optimization framework
- API integrations with legal intake software and partner systems
- AI-agent-powered internal tools and automation

*Expansion (Month 4–6):*
- AI chatbot builds for premium lead nurturing
- AI call bot infrastructure and automation
- Intake automation and documentation tools
- Website builds and redesigns for attorney partners
- Analytics dashboards for partner firms
- Case value estimator and consumer-facing tools

**Background:** 13+ years across product management, full-stack development, and growth. Senior Product Manager at TradeStation (0-1 mobile app build, 99.96% crash-free, 60% app store rating increase). Founded Summit Analytics Consulting (6-year product practice). Built MarTech stacks and lead scoring systems at Alfred Realty (67% MQL-to-SQL lift). Custom analytics dashboards at Compass. Reforge 4x alum. CXL-certified in conversion optimization. Recently built a proprietary trading platform using AI agents with Claude Code.

**Location:** Los Angeles, California

**Profile:** [01_Identity/employees/AlexDuBelko.md](../01_Identity/employees/AlexDuBelko.md)

---

### Sasha Zinshtein — Co-Founder & Performance Media

**What He Owns:**

*Core (Day 1):*
- Media buying across Meta, Google, TikTok, Snapchat, LinkedIn
- Sales execution and B2B attorney acquisition across target DMAs
- CRM setup, configuration, and daily operations
- Lead pipeline tracking, sales automation, outbound sequences
- Account relationships and partner retention
- Lead qualification, scoring operations, and routing
- Revenue reporting and dashboards
- Partner onboarding process

*Expansion (Month 4–6):*
- Ad buying / media management for partner firms' own campaigns
- Premium lead qualification and routing
- Upselling existing partners into premium leads + agency services
- Revenue expansion strategy per DMA

**Background:** 14+ years across programmatic media buying, sales, and financial underwriting. Programmatic account management for 18+ Fortune 500 brands at 33across (Bose, Coach, Diageo, Duracell, H&R Block, Indeed, Ocean Spray, PepsiCo, Tinder). Director of Growth at The Flower Agency (pioneered cannabis programmatic on Forbes, Rolling Stone). Fund Specialist at Riverpoint Capital (millions deployed in non-dilutive growth capital). Speaks capital-allocator language — ROI, payback, allocation — which is how he sells to law firms.

**Location:** New York, NY

**Profile:** [01_Identity/employees/SashaZinshtein.md](../01_Identity/employees/SashaZinshtein.md)

---

## Compensation Structure

### Profit Split
- **33% Davis Ballard**
- **33% Alex DuBelko**
- **33% Sasha Zinshtein**

Applied to all net profit after direct costs (ad spend, tool subscriptions, platform fees).

### How It Works
1. Revenue comes in from attorney partners purchasing leads at cost-per-signed-case (CPSC) pricing
2. Subtract direct costs (ad spend, platform fees, tool subscriptions)
3. Net profit splits 33/33/33

---

## How the Three Roles Work Together

```
Davis creates the ads and content that drive demand
    ↓
Alex builds the funnels and tools that capture and qualify that demand
    ↓
Sasha buys the media that distributes the creative
    and closes the attorney partnerships that monetize the leads
    ↓
Revenue flows back → 33/33/33
```

### Collaboration Map — Core Lead Gen

| Function | Davis | Alex | Sasha |
|---|---|---|---|
| **Brand & Positioning** | Owns | Supports (product alignment) | Supports (sales alignment) |
| **Ad Creative** | Creates | — | Distributes (media buying) |
| **Website & Funnels** | Provides copy | Builds & optimizes | — |
| **Quiz Flows** | Writes questions & copy | Builds & A/B tests | — |
| **CRM & Lead Scoring** | — | Architects system | Operates daily |
| **Sales Materials** | Creates decks & collateral | Builds demos/tools | Uses in outreach |
| **Analytics** | Creative performance | Infrastructure & dashboards | Revenue & pipeline |
| **Client Relationships** | Strategy presentations | Product demos | Day-to-day management |

### Collaboration Map — Expansion (Month 4–6)

| Function | Davis | Alex | Sasha |
|---|---|---|---|
| **Premium Nurturing** | Nurture copy & sequences | AI bots, chatbots, automation | Lead routing & qualification |
| **Partner Brand Identity** | Creates identity systems | — | Sells the service |
| **Partner Ad Campaigns** | Creates the creative | — | Buys the media |
| **Partner Email Flows** | Builds the flows | — | — |
| **Partner Chatbots** | — | Builds & maintains | — |
| **Partner Websites** | Copy & design direction | Builds & deploys | — |

---

*Last updated: March 20, 2026*
