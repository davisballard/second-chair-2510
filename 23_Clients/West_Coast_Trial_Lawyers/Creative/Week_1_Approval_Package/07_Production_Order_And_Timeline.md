...
|------|-------------|---------------|-------|
| **A** | Text-swap on locked scene architecture (reuse primary scene, swap text overlay + maybe one new prop shot) | ~30min per variant | 11 units |
| **B** | New scene, locked character (requires new Kling/Seedance generation) | ~1-2hr per variant with iteration | 8 units |
...
|------|---------|------|----------|
| S1 primary | Hit by a semi on I-10? | A | Elevated |
| S1-V1 | Commercial insurance is different. So is the lawyer you need. | A | Elevated |
...
| S1-V4 | Hit by a Freightliner on I-17? | A | Elevated |
...
**Totals:** Tier A = 16, Tier B = 11, Tier C = 1 → wait, that's 28. Let me recount. (Count: 6 concepts × 4-5 variants = 24 + S1-V3 duplicate UGC + M2-V1 duplicate UGC = counted separately? If V3 and V1 are single hooks produced in two registers, they count as 2 assets each. 24 hook lines; ~26 actual render units including dual-register duplicates.)
...
- ✅ Existing `Week_1_Launch_Concepts.md` archive-stamped
...
### Fri 2026-04-24
- **AM:** Run `Phase_1_Reference_Portrait_Prompts.md` in Higgsfield. Generate 4 candidates × 4 characters (Ty, Sarah, Laura, Diana).
...
### Sat 2026-04-25 — Tier A production (11 units)
- Primary scene architectures for each of the 6 concepts (rendered once, text-swap thereafter): ~2-3hr
- Text-swap variants: S1-V1, S1-V2, S1-V4, S2-V1, S2-V2, S2-V4, S5-V3, M1-V1, M1-V2, M2-V1 Elevated, M3-V2
- Time: ~4-6hr total

### Sun 2026-04-26 — Tier B production (8 units)
- New scene compositions: S1-V3 Elevated + S1-V3 UGC, S2-V3 (poetic restraint), S5-V1 (family system), S5-V2 (invisible labor), S5-V4 (2-4AM low-light), M1-V3 (group ride), M2-V1 UGC, M2-V2 (wedding-fund), M3-V1 (split-frame), M3-V3 (helmet still-life)
- Time: ~8-12hr total. If running long, defer M3-V1 or M3-V3 to Monday AM.
