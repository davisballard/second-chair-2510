...
# Push Second Chair to GitHub
# This script syncs the Second_Chair folder to a separate git repo and force-pushes it
...
# Configuration
REPO_DIR="$HOME/second-chair-2510-repo"
...
# Sync files from source to repo (excluding the push script itself and .git)
echo -e "${BLUE}📦 Syncing files...${NC}"
rsync -av --delete \
    --exclude 'push-secondchair.sh' \
    --exclude '.git' \
    --exclude '.DS_Store' \
    --exclude '.venv' \
    --exclude 'venv' \
...
if git diff --staged --quiet; then
    ...
    COMMIT_MSG="Update Second Chair - $(date '+%Y-%m-%d %H:%M:%S')"
